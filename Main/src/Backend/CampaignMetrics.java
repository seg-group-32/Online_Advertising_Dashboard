...
public class CampaignMetrics {
	...
	public void calculateMetrics(ImpressionData impressionData, ClickData clickData, ServerData serverData) {
		...
		if (clickData != null && impressionData != null) {
		...
		if (serverData != null && clickData != null && impressionData != null) {
			calculateCPA();
		}
	...
	public void calcBounceTime(ServerData serverData, Integer time) { //time in seconds
		noBounces = 0;
		for (ServerEntry serverEntry:serverData){
			if(serverEntry.getExitDate()!=null){
				long startTime = serverEntry.getEntryDate().getTime();
				long endTime = serverEntry.getExitDate().getTime();
				long timeDiff = endTime-startTime;

				if (timeDiff < time){
					noBounces = noBounces+1;
				}
			}
		}
	}

	public void calcBouncePages(ServerData serverData, Integer noPages) {
		noBounces = 0;
		for (ServerEntry s : serverData) {
			if (s.getPagesViewed() < noPages) noBounces++;
		}
	}

	public void calcBounceRate() {
		bounceRate = (float) noBounces / (float) noClicks;
	}

	//DEPENDS ON SERVER FILE LOADED
	public int getNoBounces() {
		return noBounces;
	}

	//DEPENDS ON SERVER AND CLICK FILES LOADED
